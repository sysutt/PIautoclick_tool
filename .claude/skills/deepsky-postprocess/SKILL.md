...
21. **"照搬 midtones"的前提是前面的拉伸量相同,否则就是双重拉伸。** 铁律 3 说形状可照搬,但
    ...
    **量化自检:`core − faint` = 主体动态范围,< 0.15 就是拉爆了**(C63 Ha 实测 0.742/0.859
    ...
    不要再叠 HT。
    **锚点重映射只能往"扩张"方向用,绝不能用来压亮度。** `faint` 是 p90 分位,把它往下压
    等于把"背景~p90"的**整个中间调**按同一比例压掉(NGC1499 实测压了 3.7 倍),而且样条在
    陡段前会形成**平台** → 画面上出现一块**边界硬、内部全平的惨白板**(用户一眼指出
    "亮部被压得完全没有梯度")。亮度过高必须**回上游**减小拉伸/揭示力度重做,不能事后压。
    自检办法:切一块**亮部 1:1 特写**看有没有硬边平板;全局 `coreContrast.spread` 抓不到它
    (实测那块平板反而因硬边让局部 RMS 更高 = 指标骗人)。
...
- **`bgneutral` 只吃彩色图**(它按通道采四角):单通道要做背景平移,用 `curves points`
  `[[0,0],[当前背景,目标背景],[1,1]]`。
- **`reveal`(maskstretch)+ `lift`(lmasklift)叠着用会把中间调整体抬爆。** NGC1499 实测:
  去星合成 faint 0.220 → reveal 后 0.380 → **lift 后 0.645**,主体动态只剩 0.17 → 整片发平。
  两步只挑一步用,或把 `lmasklift` 的 amount 减半;每步后必测 faint。
- **拉伸后的"亮部没梯度"先分清是电平问题还是观感问题。** 线性母版里星云通常只在 p999≈0.01
  (max=1 只是几颗星核)→ 数据**没有**被削平,梯度还在,只是被色调映射压到了视觉分辨不出的窄带。
  这种情况该用**空间局部处理 `lhe`**(铁律 6/12),不是再动全局曲线。
- **带星通道图的 `core` 是星核不是星云。** 用它判"亮部过曝"会被星核带偏(实测 SII 带星
  core=0.915 但去星后星云峰值只有 0.371)→ 判过曝/定目标电平一律用**去星图**。
- **逐通道各自拉到同一锚点会把颜色洗掉。** 通道间的强弱差异**就是**颜色:把 OIII 从 0.148
  抬到和 Ha 一样的 0.33,合成出来就是灰的。扩张曲线要在**彩色图上整体做一次**(K 曲线对三
  通道同等作用),或从最强通道推出曲线后**共用**。
