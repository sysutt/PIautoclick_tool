...
- **【pink 档必须按 O/H 强度自适应(否则 OIII 强的目标全图紫粉)】**:pink 的 B 高增益是为 **Ha 主导**目标(M17)调的;搬到 **OIII 强**目标(SH2-132)就紫粉铺满。判据 = **O/H 亮区强度比**(`lumprobe` 各通道 `anchors.core` 之比;SH2-132 实测 **1.158**,M17 ≤1)。回落公式:`B增益 = 1.25 / max(1,O/H)**3`、`B掺Ha = 0.40 / max(1,O/H)**3`。**幂次实测**:线性(B=1.08)仍紫;**pow=3(B=0.81)干净**;pow=5(B=0.60)B 掉太多偏黄。→ 取 3。**注意 O/H 不能用背景中位判**(各通道已拉伸对齐到同一背景),必须用亮区 core 锚点。
