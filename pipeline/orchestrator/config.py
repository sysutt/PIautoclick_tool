...
_DEFAULT_SETTINGS: dict[str, Any] = {
    ...
    "disc_style_source": "ref",        # 盘色风格推移的来源
                                       ...
                                       #          尺度归一、按环推倍率)。用户 2026-09-17:“星系盘面偏蓝是大共识,
                                       #          只有少数星系偏黄或白(IC342/M104),而这些共识可以从 AstroBin 拿到”
                                       #          —— 参考自带例外,不必维护例外清单。拿不到参考就不推。
                                       #   house= 自有风格形状 × 本图核电平(见 recombine.house_disc_target)
                                       #   off  = 不做。2026-09-16 曾因 M31 调坏而默认 off;回头看那是**度量和手段**
                                       #          的错(标量目标 + 全局增益 + 不尺度归一的度量),不是这件事不该做。
    "disc_push_strength": 1.0,         # 0~1:1=完全对齐参考共识,0.5=走一半(口味旋钮)
    "disc_push_lock_core": False,      # True=核心钉住不推。默认 False:参考共识本身就是“黄核蓝臂”
                                       #   的径向梯度(M74 实测核 B/G 0.93、12/12 都 <1),让核也对齐才能拿到那个暖色。
