...
请只判断这一件事:**除了星团本身的密集恒星和零散星点,画面里有没有"较大面积、成形、值得保留"的
暗云或星云结构?**

**判 true 的门槛要高、默认偏 false**(宁可漏判克制,不可误判揭示——揭示会把斑驳背景放大成脏麻点、
还会在合星时稀释星点颜色)。**以下都不算延展结构、应判 false**:
- **密集星场 / 银河恒星云**(恒星多≠星云;疏散星团常嵌在银河里,满屏星点不是"星云");
- 淡而均匀的背景辉光 / 大尺度梯度 / 天光;
- 小范围、噪声级、若有若无的暗纹。
**只有**看到**成形的暗云带/暗柱/暗球**(有明确边界与内部结构)或**明显成片的发射/反射星云**
(有颜色、有形状、占可观面积)才判 true。
...
AGENT_OPS = {
    ...
    "restretch": "**退回·用不同拉伸力度从头重跑**(不是在成片上改!成片已拉伸过、curves 只能硬压救不回层次)。"
                 "当用户说『拉伸太狠/太弱』『背景拉太亮/太花』『退回重新拉伸』『从拉伸阶段重来』『重新拉一次』时**必须用它**,"
                 "别用 curves 硬压。会用调整后的 GHS 拉伸力度把整条流程重跑一遍(母版输入约几分钟,原始素材更久)。"
                 "params:direction(weaker=拉伸减弱,背景太亮/太花时用 / stronger=加强,太暗时用)、ghs_d(可选,直接给 0~2.5 的目标力度)",
    "polybg":    "背景展平(治**平滑的**大尺度明暗·色度不均:光害渐变/暗角/天光倾斜)。逐通道拟合背景多项式"
                 "再扣除,自动按亮度剔除星点/星团/亮星云、只压天光背景。params:degree(1~3,默认2)。"
                 "**只对平滑渐变有效**;拍摄云层造成的**斑块状/不规则**不均它压不掉,那种用 graxpert_bge",
    "graxpert_bge": "GraXpert AI 背景提取(治**斑块状/不规则的云层残留、复杂背景**——polybg/gradient 那类平滑"
                 "多项式压不掉的)。用 AI+样条插值贴合不规则背景再扣除,自动避开星点/星团。约 1-2 分钟(慢)。"
                 "params:smoothing(0~1,默认0.2;越小越贴合云斑起伏、越大越平滑保守)。需装 GraXpert(配置里填路径)。"
                 "**用户提到『云』『拍的时候有云』『背景一块块脏』时优先用它**;若用户想保留大片真实弥漫云气则别用",
