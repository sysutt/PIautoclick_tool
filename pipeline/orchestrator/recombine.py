...
def star_degreen_gain(img_path: str, out_path: str, preview_path: str | None = None,
    ...
    clean_bg/starfield **根本不分星**(星色=SPCC 原样);有暗云就改走星云路线要分星,而原来这一步
    用 **SCNR 去绿,一口气铲掉星层 39% 饱和**(M80 实测 0.4064→0.2466)。绿本身是真伪影该去
    (物理上没有绿星),**错在用 SCNR 去**:它把超过 (R+B)/2 的 G 逐像素钳下去 —— 对 R>G>B 的黄橙星
    是算术必然误判(削的 70.8% 像素里 19.7% 是黄橙星),而且**逐像素钳到中点必然压扁色度**。

    **定稿做法**:用用户自己的判据「**真绿 = G 同时高于 R 和 B**」,只削 `max(0, G - max(R,B))` 的 k 倍。
    G 最低只降到 max(R,B),仍 ≥ min(R,B) → **数学上不可能制造品红**;而黄橙星(R>G>B)的 G 本就不高于
    max(R,B),超出量为 0 → **分毫不动**。这正是 SCNR 做不到的选择性。

    【★我走错过两版,记下来免得重犯】
    ① "星点锚**全通道**白平衡"(把星点总体中位拉中性):指标漂亮(饱和 0.4012/绿占优 20.4%/R:B 1.000)
       但**成片全场发蓝、连 M80 团核都蓝** —— 球状团是老年红巨星族、团核本就该偏黄。
       "平均星是白的"只对**混合星族的星场**成立;画面被一个团主导时,星点中位就是那个团**自己的真色**,
       拉成中性 = 把天体真色当色铸减掉(B 增益到 1.405)。**指标全绿但图是错的**。
    ② "G 通道全局增益"(按绿/品红对称解 0.817):团色保住了,但**用户实看"有点紫"** ——
       全局平移**必然拿绿换品红**(绿 p90 39.1%→20.8% 的同时品红 p90 0.7%→**16.7%**),
       而下游新启用的 r14b_starsat 提饱和(gain 1.5)又把这点品红一起放大到成片 19%。
       **平移治不了单侧问题;要单侧的效果就得用单侧的算子。**

    【实测对照(M80 星层;绿p90 / 品红p90 / 饱和)】
      原始(不去绿)        39.1% / 0.7% / 0.4064
      SCNR 0.8(旧法)      3.9% / 0.7% / **0.2997**
      全局增益 0.817(错)  20.8% / **16.7%** / 0.3709   ← 造出品红
      **只削真绿 k=0.8**  **7.8%** / **0.7%** / **0.3191**  ← 品红零新增、饱和比旧法高 6.5%
    (k=0.6 更保色 0.3463 但绿留 15.6%;k=1.0 绿归零但饱和 0.2864 反不如 SCNR。取 0.8。)
    返回 out_path;星点太少/本就没绿返回 None(调用方保留原图,退回 SCNR)。"""
    import numpy as np
    from xisf import XISF
    try:
        xn = XISF(img_path)
        ...
        if sg <= 0:
            return None
        m = v > (b + 5.0 * sg)                       # 真有星光的像素(判"有没有绿"只看星点)
        if int(m.sum()) < min_stars:
            if log:
                log("    星点像素仅 %d(<%d)→ 不做星层去绿" % (int(m.sum()), min_stars))
            return None
        R0, G0, B0 = a[..., 0], a[..., 1], a[..., 2]
        exc = np.maximum(0.0, G0 - np.maximum(R0, B0))          # 真绿超出量
        L = np.maximum(np.maximum(R0, G0), B0)
        _g0 = float(np.percentile((exc / np.maximum(L, 1e-6))[m], 90))
        if _g0 < 0.02:                                          # 星点本就不绿 → 不动
            if log:
                log("    星点本就不绿(真绿超出 p90 %.1f%%<2%%)→ 不动" % (_g0 * 100))
            return None
...
# 【★自有风格的盘色「形状」(用户 2026-09-16 定案)】决定性的一次实测:
#   **用户想要的色彩结构根本不在数据里**。SPCC 定标后的线性 M31 逐档几乎是平的 ——
#   R/G 1.002/1.009/1.019/1.021/1.018、B/G 0.741/0.768/0.781/0.794/0.845;
#   而他的成品是 R/G 1.163~1.269、B/G 0.815→1.376。**那条梯度是他做出来的,不是还原出来的。**
#   → 所以「颜色校准」到头也只能还原到平;要的是一次**可控的调色**。
#
#   把他 5 张手工星系各档**除以自己的核**(去掉整体色调,只看形状),形状高度一致:
#       B/G 相对核:1.00 / 1.02 / 1.13 / 1.31   (M31 1.08/1.13/1.31、M51 1.13/1.14/1.15、
#                                              M63 1.02/1.09/1.36、M65_M66 1.01/1.02/1.67)
#       R/G 相对核:1.00 / 1.02 / 1.02 / 1.01   (基本平)
#   即**核最暖、越往外越蓝**,这是真实的天体物理(核球黄、旋臂蓝),他只是把它做足。
#   验证:拿他自己 M31 的核电平 0.815 套这个形状 → 0.815/0.833/0.924/1.071,
#   对照实际 0.815/0.884/0.925/1.072 —— **几乎完全吻合**。
#
#   于是目标这样构造:**电平**由 chroma_restore 从线性(物理定标)带过来、**形状**用这个,
#   两者各管各的。好处是不需要任何外部目标、不需要按目标反号的偏置
#   (AstroBin 参考在 M31 上比用户蓝 14%、在 M51 上反过来 —— 单一偏置服务不了两端)。
HOUSE_DISC_SHAPE_B = (1.00, 1.02, 1.13, 1.31, 1.45)
HOUSE_DISC_SHAPE_R = (1.00, 1.02, 1.02, 1.01, 1.01)


def house_disc_target(img, level_scale_b: float = 1.06, level_scale_r: float = 1.15):
    """按「自有风格形状 × 本图自己的核电平」造目标廓线 → [(R/G, B/G), ...]。

    level_scale_*:核电平的整体修正 —— **用户的色调本身就比物理真值暖/浓**。实测 M31:
    色比还原后核 R/G 1.009 / B/G 0.771,用户成品 1.163 / 0.815;再算上曲线+提饱和这一段的
    欠冲(实测约 -10%),反解得 R×1.15 / B×1.18。**这两个数只在 M31 上标定过**(其它目标
    缺程序侧同口径的对照),后续有新目标对照时应复核。想整体更暖/更蓝仍用 warm/bias 偏移。
    最外档(1~3% 峰值)信噪最差,形状外推到 1.45(用户 M31 实测 1.69、M63 1.36,取保守值)。
    """
    prof = disc_color_profile(img)
    if not prof or not prof[0]:
        return None
    r0 = float(prof[0]["rg"]) * float(level_scale_r)
    b0 = float(prof[0]["bg"]) * float(level_scale_b)
    return [[round(r0 * sr, 4), round(b0 * sb, 4)]
            for sr, sb in zip(HOUSE_DISC_SHAPE_R, HOUSE_DISC_SHAPE_B)]
