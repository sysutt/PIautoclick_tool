...
def run_wbpp_stack(raw: dict, timeout: float = 3600.0, reference: str | None = None) -> str:
    ...
    if raw.get("lights"):
        ...
        # 暗场:支持**多个目录**(raw["darks"] 列表)——多晚曝光不同时(如 30s + 15s),各暗场目录都喂进去,
        #   WBPP 按曝光各建 master dark、再按曝光匹配各晚亮场(WBPP 原生按 exposure 配暗场)。单个走 raw["dark"]。
        _darks = raw.get("darks") if isinstance(raw.get("darks"), (list, tuple)) else \
                 ([raw["dark"]] if raw.get("dark") else [])
        for _dk in _darks:
            if _dk:
                args.append("dir=" + str(_dk).replace("\\", "/"))
        if raw.get("bias"):
            args.append("dir=" + raw["bias"].replace("\\", "/"))
    ...
    _has_dark = bool(raw.get("darks")) or bool(raw.get("dark"))   # nights 模式也可能是 darks 列表(多曝光)
...
def run_wbpp_stack_pernight(raw: dict, timeout: float = 3600.0) -> str:
    """智能望远镜(Dwarf 等非制冷)**多晚**原始素材:逐晚按温度/曝光匹配暗场,把**所有匹配到的暗场目录一起
    喂给单次 WBPP** —— WBPP 原生按 **exposure** 建 master dark 并匹配各晚亮场(30s 晚配 30s 暗、15s 晚配 15s 暗),
    同时**一次跑完对齐**(全晚共用 WBPP 自动选的同一参考帧)。返回 registered 目录。

    为何这样(而非"逐晚各跑一次 WBPP + 共享 referenceImage"):M54 四晚实测两坑——① 逐晚各跑→各自选参考→
    registered 尺寸/网格不同(n1-3=2180×3856、n4=2144×3680),整合 executeGlobal 失败;② 改用外部
    referenceImage 对齐→自定义 WBPP 报 "Empty output file name"、大面积配准失败(第2晚 8/61、第3晚卡死)。
    真需求其实是**曝光不同要配不同暗场**(温度通常只差 1-2°,WBPP 读不到 Dwarf 的 DET-TEMP 也无妨),而 WBPP
    按曝光配暗场是原生能力 → **单次跑 + 多暗场**既对齐正确又暗场正确。
    **局限**:同曝光但温度差很大的暗场,WBPP 只按曝光合并、不按温度区分(需两阶段,暂不做)。
    需 raw['calib_library'] + 多晚;单晚/无库自动退回单次 run_wbpp_stack。
    """
    import os
    ...
    if len(nights) <= 1 or not lib or not os.path.isdir(lib):
        return run_wbpp_stack(raw, timeout)          # 单晚/无库 → 单次即可

    from . import calib_match
    pn = calib_match.auto_calib_pernight(
        lib, [n["light"].replace("\\", "/") for n in nights], kinds=("dark",), log=print)
    # 收集各晚匹配到的暗场目录(去重保序)→ 全喂单次 WBPP,WBPP 按曝光各配各的
    darks, seen = [], set()
    for i in range(len(nights)):
        d = (pn[i]["dark"]["dir"].replace("\\", "/")
             if (i < len(pn) and pn[i].get("dark")) else None)
        if d and d not in seen:
            seen.add(d); darks.append(d)
    if not darks and (raw.get("dark") or "").strip():
        darks = [raw["dark"].strip().replace("\\", "/")]
    print("== 多晚(%d 晚)WBPP:单次跑 + 喂 %d 个匹配暗场(WBPP 按曝光各配各的);全晚一次对齐到同一参考 ==" %
          (len(nights), len(darks)))
    for d in darks:
        print("   暗场:%s" % os.path.basename(d))
    raw2 = dict(raw)
    raw2["darks"] = darks
    raw2.pop("dark", None)
    return run_wbpp_stack(raw2, timeout)
...
def run_integrate(registered_dir: str, out_path: str | None = None,
    ...
    **默认开启去线**(`trail_reject=True`):Winsorized sigma(sigma_low/high 4.0/2.8)
    ...
    # 【一次性整合全部帧(用户 2026-09-07 定)】此前的"按像素预算分批 + 失败二分重试"是把整合失败**误诊**为
...
def run_rgb(input_path: str, timeout: float = 600.0,
    ...
    #   不受此影响。星团 0.42×/纯亮场 0.75×/星系 0.72× 不动。
    tb = (0.42 if cluster_mode else 0.75 if lights_only else 0.72 if _galaxy else 0.82) * PEAK_BG
    ...
    # 【M42 亮核四合星·自动流程未解(用户 2026-09-09 决定放弃)】试过弱拉伸提前分离(四合星连成一团 SXT 分不开)、
    #   brightcore 仿手动 3-HT 拉伸(保住核但外围被下游 bgneutral 抬起=断层;跳过 bgneutral/HDR 又变平白饼)——
    ...
    if _starfield:
        ...
        # 【真发射星云:GHS 拉伸 + 饱和进一点 + 揭示减弱(用户 2026-09-07 M16 反馈)】排除干净背景/星系/局部星云后的
        #   真弥散发射星云 → GHS 拉伸(抬星云核心)+ 星云饱和各提一点(评委闭环兜底)。**揭示 reveal_d 减半**:短曝低
    ...
    # 【★只在 SPCC 没跑成时才做(用户 2026-09-14 复跑实测,血的教训)】把天文解修好、SPCC 真正生效之后,
    #   颜色本来就是对的 —— 实测 r09_dn2(SPCC 后)星系核 R-G +1.7% / B-G -4.4%、核心绿占优仅 1.4%。
    #   而这步的目标值取自参考图的**成片**(已经过星点饱和等处理,G 自然更高),拿来校我们的**中间阶段**
    #   两者不可比:它把 G 硬推了 +10.6% → 本体绿占优 15.9%→99.9%、核心 1.4%→100%,
    #   再被 rG_bodysat 的 +0.40 放大成 R-G -39.8% / B-G -66.6% = **三个星系核心一片鲜绿**。
    #   → 按用户定的原则"星系校色主要依靠 BN-CC 或 SPCC":SPCC 成了就**完全信它**,这步只当
    #   退到 BN+CC 时的补救(那种情况下实测有效:星系核 B-G -10.8/-5.0/-5.8% → -3.9/+2.1/+1.2%)。
    _star_wb_done = False
    if method != "spcc" and _ref_tg and _ref_tg.get("star_balance"):
        try:
    ...
    # 【★顺序:校色 → **定色相** → 再提饱和(用户 2026-09-15「要把色彩调到我手调的水平」)】
    #   本步必须在 rG_chromarestore 之后(还原的作用就是把颜色拉回线性真值,放它前面的审美偏移会被擦掉),
    #   且必须在 **rG_bodysat 之前**:HSV 提饱和压低的是**最小通道**,而我们盘区的最小通道正是 B —— 
    #   实测 M63 提饱和把盘区信号 B/G 从 0.814 压到 **0.704**,排在后面的偏蓝只能把它拉回 0.800 = 做无用功。
    #   先把色相定对,盘区就变成 R 1.18 / G 1.00 / B 1.01(最小通道是 G),提饱和的作用方向随之改变。
    #   另:用户手工成片的本体饱和其实**比我们低**(0.141 vs 0.195),观感更有色彩靠的是色相不是饱和度。
    ...
    if _galaxy:
        try:
            ...
            try:
                ...
                _bias = 0.0
            # 上限由 ±0.06 放宽到 ±0.15(用户 2026-09-15):实测要够到用户手工成片的蓝,
            #   M63 各信号档需要 gB 1.11~1.20,而 ±0.06 最多给到 gB/gG=1.093 —— 卡在够不到的地方。
            #   反解:gB/gG=1.15 对应偏置 0.095。
            try:
                _warm = float(config.get_setting("disc_warm_bias") or 0.0)
            except (TypeError, ValueError):
                _warm = 0.0
            # 【二维风格向量(用户 2026-09-15「要把色彩调到我手调的水平」)】一个「偏蓝」标量不够:
            #   实测 M63 从色比还原后的 [R/G 1.080, B/G 0.819] 到用户手工版的 [1.160, 0.950],
            ...
            if True:
                ...
                # 【★盘色目标:按目标取 AstroBin 同视场参考(用户 2026-09-15 拍板)】
                #   单一全局目标服务不了所有星系 —— 用户手工库真盘 B/G 从 0.87(M31 尘埃暖调)
                #   到 1.16(M51 蓝旋臂),是真实的类型差异。M51 的 12 张同视场参考中位
                #   [1.031, 1.015],M31 的是 [1.335, 0.917] —— 各自都对上了用户手调的那一版。
                # 【拿不到参考就**不推**(用户 2026-09-15 M31「颜色偏紫」的教训)】
                #   那一跑 10/12 张参考量不出来(背景电平估计在天体占满画面时失效,已修)→ 样本量 2
                #   → 退回家族中位 [1.21, 0.99],而 M31 真盘是 0.87 → B 被硬推上去 28%,
                #   再被 rG_bodysat 放大 = R≈B>G 洋红。
                #   **教训:没有针对这个天体的证据时,"退回一个平均值"比"什么都不做"更危险** ——
                #   SPCC + 色比还原给出的已经是物理上对的颜色(M31 那一跑是 0.857,离真值 0.87 只差 1.5%),
                #   拿一个谁都不像的平均值去覆盖它,只会把本来对的推歪。→ 没参考就只留用户自己的偏暖/偏蓝。
                _sty = None
                _sty_src = ""
                _rt = (_ref_tg or {}).get("disc_signal")
                _rtn = int((_ref_tg or {}).get("disc_signal_n") or 0)
                # 样本量是唯一有用的筛法(按纯宽带/奖项/曝光筛都试过没用,见 [[pi-galaxy-disc-color-target]])
                if _rt and len(_rt) >= 2 and _rtn >= 6:
                    _cR, _cB = float(_rt[0]), float(_rt[1])
                    # 兜一道明显离谱的情况(参考里混进单色/伪彩/强 Ha 增强会把中位带跑)
                    if 0.80 <= _cR <= 1.60 and 0.70 <= _cB <= 1.40:
                        _sd = (_ref_tg or {}).get("disc_signal_sd") or [0.0, 0.0]
                        _sty_src = f"AstroBin 同视场 n={_rtn}(σ {float(_sd[0]):.3f}/{float(_sd[1]):.3f})"
                        # 预补偿下游漂移:风格步之后到成片实测 R ×1.043 / B ×0.952
                        _sty = (_cR / 1.043, _cB / 0.952)
                    else:
                        print(f"  [盘调色] 参考盘色 [{_cR:.3f}, {_cB:.3f}] 超出合理范围 → 不做风格推移")
                # 配置里显式写了 galaxy_style_target 才覆盖(留空 = 按目标取参考;没参考就不推)
                try:
                    _hs = config.get_setting("galaxy_style_target")
                    if isinstance(_hs, (list, tuple)) and len(_hs) >= 2:
                        _sty = (float(_hs[0]) / 1.043, float(_hs[1]) / 0.952)
    ...
    #   (MTF 把中盘抬暖)②还原会把刚提上去的饱和部分抵消。放在风格偏置之后的理由见上一块。
    ...
    if recombine_stars and not _starfield:
        ...
        # 【目标 = 用户手工库的中位,不是"甜区中心"(2026-09-19 M77「星点饱和又有点过高」)】
        #   旧值 0.25 来自早年**通用甜区 0.22~0.40 的中心**,跟用户口味无关 —— 他自己五张手工成片
        #   实测 s_star 0.147~0.229、**中位 0.180**。以前这个错值没暴露,是因为两件事互相抵消:
        #   ①导出是 4:2:0,交付文件比管线量到的低三成;②闭环常年顶在增益封顶 2.5 也够不到 0.25。
        #   把导出改成 4:4:4 之后管线量到的数 ≈ 用户看到的数,这个偏高就直接现形了。
        #   M77 实测(交付 4:4:4 分项 vs 用户手工库 蓝S 0.201~0.233 / 黄S 0.204~0.253):
        #     gain 2.5(旧,顶封顶) r13 0.208 → 蓝S 0.239 黄S 0.295  ← 黄星超出 17%
        #     gain 2.2            r13 0.183 → 蓝S 0.214 黄S 0.273
        #     gain 2.0            r13 0.167 → 蓝S 0.199 黄S 0.253   ← 两项都贴区间边
        #   容差同时从 ±0.03 收到 ±0.02:±0.03 允许落点在 0.22~0.28 乱跑(成片 0.21~0.26),
        #   比这次要调的幅度还大,等于把目标值调准了也守不住。
        _ss_target = 0.18          # 成片 s_star 目标 = 用户手工库中位(见 [[pi-star-color-benchmark]])
        ...
        for _qit in range(4):
            ...
            try:
                ...
                _ss = 0.0
            if _ss <= 0 or abs(_ss - _ss_target) <= 0.02:               # 达标(双向容差)→ 停
                break
