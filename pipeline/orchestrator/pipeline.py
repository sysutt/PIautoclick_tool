    ...
    if _galaxy:
        try:
            ...
            if True:
                ...
                #   起因:M51 成片一直发灰。查下来是**度量**坏了(disc_signal_color 的取样 53% 落在星场、
                #   0% 落在星系),它把用户手工库读成 B/G 0.87 → 家族目标写着「把蓝压到 0.87」→ 越闭环越偏。
                #   口径修好后重量,用户手工库真盘是 M51 [1.01, 1.16] / M63 [1.21, 1.01] / M64 [1.23, 0.99] /
                #   M65_M66 [1.12, 0.97] / M31 [1.32, 0.87] —— 中位 [1.21, 0.99]。
                #   但 **B/G 0.87(M31 尘埃暖调)到 1.16(M51 蓝旋臂)是真实的类型差异,不是噪声**:
                #   单一全局目标服务不了两端。12 张 M51 同视场参考实测中位 [1.031, 1.015],家族中位是
                #   [1.21, 0.99] —— **R/G 差 0.18**,这才是 M51 发灰的大头。
                #   → 优先用**这个天体自己的**参考色;拿不到(没解析出坐标/后端没作品/样本太少)才退回家族中位。
                #   目标源和成片侧用**同一个函数**量(quality.ref_targets 里调的就是 disc_signal_color),
                #   否则两个口径之间的系统偏差会原封不动变成成片色偏。
                _hR, _hB = 1.21, 0.99            # 兜底:用户手工库中位(修好口径后重量)
                _sty_src = "家族中位(没拿到同视场参考)"
                ...
                #   少于 6 张时单张异常拉得动中位 → 不如用家族中位。
                if _rt and len(_rt) >= 2 and _rtn >= 6:
                    _cR, _cB = float(_rt[0]), float(_rt[1])
                    # 兜一道明显离谱的情况(参考里混进单色/伪彩作品会把中位带跑)
                    if 0.80 <= _cR <= 1.50 and 0.75 <= _cB <= 1.40:
                        _sd = (_ref_tg or {}).get("disc_signal_sd") or [0.0, 0.0]
                        _hR, _hB = _cR, _cB
                        _sty_src = f"AstroBin 同视场 n={_rtn}(σ {float(_sd[0]):.3f}/{float(_sd[1]):.3f})"
                    else:
                        print(f"  [盘调色] 参考盘色 [{_cR:.3f}, {_cB:.3f}] 超出合理范围 → 退回家族中位")
                # 用户在配置里显式写了 galaxy_style_target 才覆盖上面两者(留空 = 自动按目标取)
                try:
                    ...
                    pass
                print(f"  [盘调色] 盘色目标 R/G {_hR:.3f} B/G {_hB:.3f}  ← {_sty_src}")
                # 预补偿下游漂移:风格步之后到成片实测 R ×1.043 / B ×0.952(新口径复测 1.041/0.947,一致)
