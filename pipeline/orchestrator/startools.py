...
def denoise_fit(in_fit: str, out_fit: str, *, tag: str = "chan", timeout: float = 1800.0, log=print) -> str:
    ...
    try:
        ...
        log(f"[stars] NXT 前置降噪失败({repr(e)[:60]})→ 退 DeepSNR")
    try:
        if siril.deepsnr_exe():
            bo = os.path.splitext(os.path.basename(out_fit))[0]
            siril.deepsnr(in_fit, os.path.join(R, bo), model=2, stride=480, timeout=timeout)
            if os.path.exists(out_fit):
                return out_fit
    except Exception as e:
        log(f"[stars] DeepSNR 前置降噪失败({repr(e)[:60]})→ 跳过")
    return in_fit
