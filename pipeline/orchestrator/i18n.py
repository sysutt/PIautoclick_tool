...
# 【补齐英文覆盖(用户 2026-09-11)】 EN 模式下未收录的串会回落中文,界面中英混杂,
#   用户会误判当前语言。此处补上 app_ui 里剩余的 t() 中文串。
ZH_EN.update({
    '[暂停] 继续。': '[Paused] Resuming.',
    '[准备] 等待 PixInsight / job-runner 就绪…': '[Setup] Waiting for PixInsight / job-runner to come up...',
    '[✗] PixInsight/job-runner 未能在 90s 内就绪,已放弃。请检查 PI 路径或手动启动。': '[X] PixInsight/job-runner did not come up within 90s; giving up. Check the PI path or start it manually.',
    '[准备] runner 已就绪,开始处理。': '[Setup] Runner ready; starting.',
    '[✗] 无 PI SHO 暂不支持从原始素材直接叠加(per-filter 需按滤镜分组)。': '[X] PI-free SHO cannot stack straight from raw frames yet (per-filter needs grouping by filter).',
    '[叠加] 无 PI 原始素材 → Siril stack_engine 叠加(零 PixInsight,不走 PI WBPP)…': '[Stack] Raw frames, PI-free -> stacking with the Siril engine (no PixInsight, no WBPP)...',
    '[叠加] 黑白 per-filter 原始素材 → WBPP(读真实 FILTER 按滤镜分组校准对齐)…': '[Stack] Mono per-filter raw frames -> WBPP (reads the real FILTER keyword, calibrating and aligning per filter)...',
    '[叠加] 原始素材 → 自定义滤镜法 WBPP(校准+去马+对齐)…': '[Stack] Raw frames -> WBPP with custom filter tags (calibrate + debayer + align)...',
    '[筛帧] 残差法检测卫星/飞机线…': '[Cull] Detecting satellite/aircraft trails from residuals...',
    '[筛帧] 含轨迹帧过多,为保信噪未剔。': '[Cull] Too many frames contain trails; kept them to preserve signal-to-noise.',
    '[筛帧] 未检出轨迹。': '[Cull] No trails found.',
    '[筛帧] 逐帧背景检测有云/低透明度帧…': "[Cull] Checking each frame's background for cloud / poor transparency...",
    '[筛帧] 无离散云帧(或比例过高未剔,可能整晚透明度差)。': '[Cull] No isolated cloudy frames (or too many to drop - transparency may have been poor all night).',
    '[筛帧] 未检出云帧。': '[Cull] No cloudy frames found.',
    '[后期] 无暗场 → 干净背景模式(不揭示/压背景)': '[Process] No darks -> clean-background mode (no reveal, background kept down)',
    '[后期] 智能望远镜 → 星点加强去绿(SCNR 0.8);蓝弱保持 SPCC 真彩不硬补': '[Process] Smart telescope -> stronger green removal on stars (SCNR 0.8); weak blue left as SPCC true colour, not forced',
    '载入中…': 'Loading...',
    '看大图(细看云/梯度/拖线)': 'View full size (inspect cloud / gradient / trailing)',
    '背景 —': 'Background -',
    '读取失败': 'Read failed',
    '背景 {:.3f}': 'Background {:.3f}',
    '背景中值 {:.4f} · 解拜耳 {} · 显示 {}×{}': 'Background median {:.4f} - debayer {} - shown {}x{}',
    '预览失败:{}': 'Preview failed: {}',
    '↩ 恢复保留此帧': 'Keep this frame after all',
    '✕ 剔除此帧': 'Drop this frame',
    '筛帧 · 剔除坏帧(云/梯度/拖线/跑焦)': 'Cull frames - drop bad ones (cloud / gradient / trailing / out of focus)',
    '⚑ 自动标记背景异常': 'Auto-flag unusual backgrounds',
    '按背景中值离群(云/月光/梯度会显著抬高背景)自动预标剔除,供你复核。': 'Pre-flags frames whose background median is an outlier (cloud, moonlight and gradients raise it noticeably) so you can review them.',
    '全部保留': 'Keep all',
    '反选': 'Invert selection',
    '机内叠加': 'In-camera stack',
    '缩略图已全部载入({} 帧)': 'All thumbnails loaded ({} frames)',
    '正在解拜耳预览… {}/{}': 'Debayering previews... {}/{}',
    '筛帧': 'Cull frames',
    '缩略图还没载入够(需要背景数据)。请等进度条走完再点。': 'Not enough thumbnails loaded yet (background data is needed). Wait for the progress bar to finish.',
    '已按背景离群预标 {} 帧(阈值 {:.3f})。\n请逐一复核后再确定——自动标记只是线索。': 'Pre-flagged {} frames as background outliers (threshold {:.3f}).\nReview each one before confirming - the auto-flags are only a hint.',
    '共 {} 帧 · 保留 {} · 剔除 {}': '{} frames - keeping {} - dropping {}',
    '确定(新剔除 {} · 恢复 {})': 'OK (drop {} - restore {})',
    '确定': 'OK',
    '执行失败:{}': 'Failed: {}',
    '+ 添加已叠加母版': '+ Add a stacked master',
    '每行 = 一个已叠加母版 + 滤镜标签。IR-UVcut→RGB 底;': 'One row = one stacked master + its filter tag. IR-UVcut -> the RGB base;',
    '整合前对对齐子帧做两道质量筛选:\n': 'Two quality checks on the aligned frames before integration:\n',
    '窄带母版在上方「素材」里加一行、滤镜标 Hα/OIII 等即可融合;此处只调融合强度。无窄带行 = 纯 RGB': 'To blend narrowband, add a row under Source above and tag it Ha/OIII etc.; this only sets the blend strength. No narrowband row = plain RGB.',
    '成片导出到这个目录,文件名自动用项目名(如 M54_260712_D3);点『导出成片』直接存、不弹窗。\n': 'The finished image goes to this folder, named after the project (e.g. M54_260712_D3); Export saves straight there without a dialog.\n',
    '只跑到选定步骤,产物导出到输出目录,后续你在 PixInsight 手工接管。\n': 'Stops at the step you pick and writes the result to the output folder so you can take over by hand in PixInsight.\n',
    '窄带合成方式(覆盖相机默认):SHO=三通道哈勃(需 SII+Ha+OIII);': 'How narrowband is combined (overrides the camera default): SHO = three-channel Hubble palette (needs SII + Ha + OIII);',
    '配色是主观档 → 默认四种都生成供你挑:\n': 'Palette is a matter of taste -> all four are generated by default so you can pick:\n',
    '勾选:SHO 全程零 PixInsight(Siril 整合 + StarNet2 去星 + GraXpert/DeepSNR AI 降噪\n': 'Tick: run SHO entirely without PixInsight (Siril integration + StarNet2 star removal + GraXpert/DeepSNR AI denoise\n',
    '无 PI 引擎调色预设(比例控制器旋钮组):\n': 'Colour presets for the PI-free engine (a set of ratio controls):\n',
    '勾选:纯 RGB 全程零 PixInsight(Siril 真 SPCC 光度校色 + GHS 压亮核 +\n': 'Tick: run plain RGB entirely without PixInsight (real SPCC photometric colour calibration in Siril + GHS core compression +\n',
    '无 PI RGB 引擎预设:\n': 'Presets for the PI-free RGB engine:\n',
    '背景梯度提取(无 PI RGB,线性阶段 subsky):\n': 'Background gradient extraction (PI-free RGB, subsky at the linear stage):\n',
    '星云区揭示强度(无 PI RGB):护亮核+护背景,只提暗弱/中间调星云。\n': 'How strongly faint nebula is revealed (PI-free RGB): protects the bright core and the background, lifting only faint and mid tones.\n',
    '残留辉光清除(成片后 ABE 式,补线性去梯度漏掉的局部残留辉光+色偏,如角落 amp glow/光污染的品红角)。\n': 'Removes leftover glow (an ABE-style pass on the finished image, catching local glow and colour casts that linear gradient removal missed - e.g. amp glow or a magenta corner from light pollution).\n',
    '勾选:HOO 双窄带全程零 PixInsight(Siril 提取 Ha/OIII + 线性 GraXpert 去梯度 +\n': 'Tick: run dual-narrowband HOO entirely without PixInsight (Ha/OIII extraction in Siril + GraXpert gradient removal while linear +\n',
    '无 PI HOO 引擎预设:\n': 'Presets for the PI-free HOO engine:\n',
    '暗星云(象鼻/尘柱/暗带)内部层次常被压成死黑 → 提亮中间调揭示。\n': 'Dark nebulae (trunks, dust pillars, dark lanes) often get crushed to flat black -> lifts the mid tones to bring their structure back.\n',
    '自适应=去绿 + 黄区加红 + 提饱和,偏自然暖调(默认,推荐)。\n': 'Adaptive = remove green, add red in the yellows, raise saturation; leans naturally warm (default, recommended).\n',
    'DarkStructureEnhance 原生复刻:蒙版内压暗,加深暗尘/暗带、提升立体感。\n': 'A faithful take on DarkStructureEnhance: darkens inside a mask to deepen dust and dark lanes and add depth.\n',
    '处理结束后自动停 runner/看门狗并结束 PI,把 PixInsight 交还给你。\n': 'When processing finishes, stops the runner and watchdog and closes PI, handing PixInsight back to you.\n',
    'GHS 拉伸后让 LLM 评委对照判断力度是否合适;\n': 'After the GHS stretch, lets the AI reviewer judge whether the strength is right;\n',
    'maskstretch(lum 蒙版+bgProtect):额外拉伸只作用在暗弱/中间调,\n': 'maskstretch (luminance mask + background protection): the extra stretch only touches faint and mid tones,\n',
    '按确定性质量指标一键补救(纯 numpy,秒出):背景偏色→中和;星点发闷→星蒙版提饱和。\n': 'One-click fixes driven by the measured quality numbers (pure numpy, instant): colour-cast background -> neutralised; dull stars -> saturation raised through a star mask.\n',
    '有天文解析时,用 AnnotateImage 标注 Messier/NGC/IC/SH2 + HIP/TYC/GAIA 恒星,\n': 'When a plate solve is available, labels Messier/NGC/IC/SH2 objects and HIP/TYC/GAIA stars with AnnotateImage,\n',
    '结束 PixInsight 并冷启动,加载**最新的 job-runner.js**(改了 runner 脚本后点它生效;\n': 'Closes PixInsight and starts it fresh so the latest job-runner.js is loaded (click this after editing the runner script;\n',
    '生成一个独立小脚本,让你在**自己平时的 PixInsight** 里手动处理完后运行一次,\n': 'Writes a small standalone script that you run once in your own PixInsight after finishing by hand,\n',
    '新建项目(会先自动保存当前工程)': 'New project (the current one is saved first)',
    '无预览': 'No preview',
    '选一个文件夹,按 FITS 头+文件名自动识别亮场/暗场/机内成片等 → 回填下面字段;': 'Pick a folder and lights, darks, in-camera stacks and so on are recognised from the FITS headers and filenames, then filled in below;',
    ...
    '请先填/选这组的亮场目录(含原始单帧)。': "Please choose this group's lights folder first (it must hold the raw sub-frames).",
    '扫描失败:{}': 'Scan failed: {}',
    '该目录下没找到原始单帧(校准场已排除;若是母版/对齐子帧请用对应模式)。': 'No raw sub-frames found in this folder (calibration frames are excluded; for masters or aligned frames use the matching mode).',
    '未匹配到符合硬性条件(曝光/增益/滤镜/尺寸)的校准场。\n': 'No calibration frames matched the required exposure, gain, filter and size.\n',
    '请至少填一个已叠加母版文件。': 'Please provide at least one stacked master file.',
    '母版文件不存在:{}': 'Master file not found: {}',
    'AI: 思考中…（0s）': 'AI: thinking... (0s)',
    '未检测到 GraXpert。请在『配置』里把 graxpert_path 指向 GraXpert.exe(装默认位置可自动识别)。': 'GraXpert not found. Point graxpert_path at GraXpert.exe under Settings (a default install is detected automatically).',
    'AI: 正在处理中,请等这次跑完再重新拉伸。': 'AI: still working - wait for this run to finish before stretching again.',
    '(收到,无需改图)': '(Understood - no change needed.)',
